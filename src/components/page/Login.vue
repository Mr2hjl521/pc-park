...
<script>
	export default {
		...
		created(){
			...
			if(localStorage.getItem("che")){
                this.checked=true
			}else{
				this.checked=false
			}

			if(localStorage.getItem("cheeA")){
				...
				this.$axios({
					...
					if (response.status <= 200) {
						if (response.data.code == 0) {
							...
							localStorage.setItem("realName",response.data.user.realName)
							localStorage.setItem("site",response.data.user.province+response.data.user.city+response.data.user.district)
							if(res.checked==true){
								     res.applyGood.push({
								...
								localStorage.setItem("che",true)
							}else  {
								//没有填写记住密码
								localStorage.removeItem("che");
							}

							if(res.checkeders==true){
								localStorage.setItem("cheeA",true)
                                localStorage.setItem('checkedersid',res.ruleForm.username)
							}else{
								//没有记住用户名
								localStorage.removeItem("cheeA");
							}
							res.$router.push('/404');
