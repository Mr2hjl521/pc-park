    ...
    <div class="whiteListManager">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i>黑名单</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
                ...
                <el-table-column label="操作" align="center" class-name="table">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" icon="el-icon-delete" class="red" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
                          <el-button type="text" icon="el-icon-star-off" @click="attention(scope.$index, scope.row)">关注</el-button>
                        <el-button type="text" icon="el-icon-star-off" @click="noattention(scope.$index, scope.row)">取消关注</el-button>
                    </template>
                </el-table-column>
                <el-table-column prop="name" label="姓名" class-name="table">
                </el-table-column>
                <el-table-column prop="lincensePlateNumber" label="车牌号" class-name="table">
                </el-table-column>
                <el-table-column prop="describtion" label="描述" class-name="table">
                </el-table-column>
                <el-table-column prop="createTime" :formatter="dateFormatter" label="创建时间" sortable class-name="table">
                </el-table-column>
                 <el-table-column prop="isFlag"  label="关注" class-name="table">
                </el-table-column>
                 <el-table-column prop="listType"  label="备注" class-name="table">
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="添加" :visible.sync="addVisible" width="30%">
            <el-form ref="addForm" :model="addForm" :rules="carFormRules" label-width="100px" >
                ...
                <el-form-item label="车牌号" prop="lincensePlateNumber">
                 ...
                 <el-form-item label="是否关注" prop="isFlag">
                    <el-select v-model="addForm.isFlag" placeholder="类型" class="handle-select mr10">
                        <el-option key="1" label="否" value="1"></el-option>
                        <el-option key="0" label="是" value="0"></el-option>
                    </el-select>
                </el-form-item>
    ...
    export default {
            ...
            return {
                ...
                carFormRules:{
                    ...
                    name:[
                        ...
                        {min: 2, max: 5, message: '长度应在2个字到5个字之间', trigger: 'blur'},
        ...
        methods: {
            ...
            getData(value) {
                ...
                var res = this;
                this.$axios({
                    url: 'blackList/selectBlackListAll',
                    method: 'post',
                ...
                if (response.status <= 200) {
                    res.tableData=[]
                    res.tableData = response.data.blackListData;
                    res.totalRecords=response.data.blackListTotalRecords; //总条数
                }
            ...
            attention(index, row) {
                ...
                this.attForm = {
                    ...
                    isFlag:row.isFlag,
                    describtion:row.listType
                }
                var setData=this.attForm
                this.$axios({
                            url: 'blackList/updateFlagZero',
                            method: 'post',
                            data:setData
                        }).then(function (response) {
                        if (response.status <= 200) {
            ...
            search(value) {
                ...
                var res = this;
                this.$axios({
                    url: 'blackList/searchBlackList',
                    method: 'post',
                    data:{"keyWord":res.select_word,"pageNum":res.cur_page,"pageSize":res.pagesize,"parkId":Number(localStorage.getItem('parkId'))}
                }).then(function (response) {
                if (response.status <= 200) {
                    console.log(response)
                    res.tableData = response.data.blackListData;
            ...
            saveEdit() {
                ...
                this.$refs.editForm.validate(valid => {
                    if (!valid) {
                        ...
                        this.$axios({
                            url: 'blackList/updateByPrimaryKey',
                            method: 'post',
                            data:setData
                        }).then(function (response) {
                        if (response.status <= 200) {
            ...
            deleteRow(){
                var res = this;
                this.$axios({
                    url: 'blackList/deleteByPrimaryKey',
                    method: 'post',
                    data:{id:res.id}
                }).then(function (response) {
                if (response.status <= 200) {
